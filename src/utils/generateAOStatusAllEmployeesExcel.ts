...
/**
 * Isolated service to generate the AO Status All Employees Excel buffer.
 */
export default async function generateAOStatusAllEmployeesExcel(data: AOStatusExportData): Promise<Buffer> {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('AO Status All Employees');
  ...
  const colDefs = [
    ...
    { key: 'position', label: 'Position', width: 18, show: visibleColumns.position !== false },
    { key: 'motherUnit', label: 'Mother Unit', width: 25, show: visibleColumns.motherUnit !== false },
    { key: 'detailedOffice', label: 'Detailed/Designated Office/Hospital', width: 25, show: visibleColumns.detailedOffice !== false },
    { key: 'designatedPosition', label: 'Designated Position/Function', width: 20, show: visibleColumns.designatedPositionFunction !== false },
    { key: 'recalledFrom', label: 'Recalled From', width: 15, show: visibleColumns.recalledFrom !== false },
    { key: 'recalledTo', label: 'Recalled To', width: 15, show: visibleColumns.recalledTo !== false },
