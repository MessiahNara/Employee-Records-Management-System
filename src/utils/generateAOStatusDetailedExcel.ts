...
/**
 * Isolated service to generate the AO Status Detailed Excel buffer.
 */
export default async function generateAOStatusDetailedExcel(data: AOStatusExportData): Promise<Buffer> {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('AO Status Detailed');
  ...
  const colDefs = [
    ...
    { key: 'detailedOffice', label: 'Detailed/Transferred Office/Hospital', width: 25, show: visibleColumns.detailedOffice !== false },
