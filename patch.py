...
file_path = 'server/src/routes/inventory.routes.ts'
with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

target = '''router.delete('/disposal-history/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
      ...
      if (isStorageLog) {
        ...
        if (!isNaN(yearToRemove)) {
          ...
          if (!currentDates.includes(yearToRemove.toString())) {
            // Add it back
            currentDates = currentDates ? ${currentDates},  : yearToRemove.toString();
            // Sort dates
...
replacement = '''router.delete('/disposal-history/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
